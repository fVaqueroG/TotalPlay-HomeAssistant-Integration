/* Simulate TMDB logos without a real TMDB key, network, or Home Assistant. */
const assert = require('node:assert/strict');
const fs = require('node:fs');
const path = require('node:path');
const vm = require('node:vm');
const root = path.join(__dirname, '../custom_components/totalplay_stb/www');
const loader = fs.readFileSync(path.join(root, 'totalplay-stb-v037.js'), 'utf8');
...
assert.match(loader, /totalplay-guide-layout-v036\.js\?v=0\.3\.7/);
assert.match(loader, /totalplay-app-logos\.js\?v=0\.3\.7/);
...
const providers=[{provider_id:1,provider_name:'Max',logo_path:'/max.png'}, {provider_id:2,provider_name:'Max Amazon Channel',logo_path:'/wrong.png'}];
assert.equal(find({name:'Max'},providers),'https://image.tmdb.org/t/p/w154/max.png');
assert.equal(find({name:'YouTube'},providers),null);
...
const card=new Card();
card.setConfig({title:'Totalplay',tmdb_api_key:'TEST_READ_ONLY_KEY'});
assert.equal(card.shadowRoot.querySelector('.tp-version-badge').textContent,'Card v0.3.7');
assert.equal(card._appsAll.children[0].querySelector('.app-logo').hidden,false,'Text shown until provider logos load');
card._switch('apps');
(async()=>{
  ...
  assert.equal(card._appsAll.children[4].querySelector('.app-logo-image'),null,'YouTube is not YouTube Premium');
  assert.equal(card._appsAll.children[5].querySelector('.app-logo-image'),null,'Unlisted provider retains text');
  card._appsAll.children[0].fire('click');assert.equal(card.played,'333','App click retains launch command');
  ...
  console.log('PASS: TMDB logo aliases, image safety and fallback, Apps launches, provider caching, editor and frontend version');
