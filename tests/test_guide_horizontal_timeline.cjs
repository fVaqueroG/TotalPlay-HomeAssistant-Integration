...
card._renderGuide();
assert.equal(parseFloat(props['--tp-slot-width']),250,'Four half-hours fill the guide viewport');
assert.ok(times.children.length>=12,'Six hours of programme slots are rendered');
assert.equal(parseFloat(props['--tp-total-width']),times.children.length*250);
...
card._renderGuide();
assert.equal(parseFloat(props['--tp-slot-width']),100,'Resize preserves two-hour window');
