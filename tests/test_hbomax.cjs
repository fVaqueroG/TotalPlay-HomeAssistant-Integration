...
assert.deepEqual(Array.from(card.usedProviders),['Max Amazon Channel','Max'],'HBO Max artwork is preferred over legacy Max');
...
card._renderApps();
assert.deepEqual(Array.from(card.usedProviders),['Max'],'Legacy Max art remains a valid fallback');
