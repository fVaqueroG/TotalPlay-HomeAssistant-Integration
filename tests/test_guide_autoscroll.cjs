/* Check automatic paging, scroll state and version badge without Home Assistant. */
...
class Element {
  constructor() {
    this.listeners = new Map(); this.hidden = false; this.scrollTop = 0;
    this.scrollLeft = 0; this.scrollHeight = 2450; this.clientHeight = 400;
    this.children = [];
  }
  ...
  querySelector(selector) { return selector === '.tp-version-badge' ? this.children.find(c => c.className === 'tp-version-badge') || null : null; }
}
...
// Simulated DOM establishes actual row height and available viewport height.
...
Card.prototype.setConfig = function (config) {
  this._config = config; this._limit = 35; this._tab = 'guide';
  this._scroll = new Element(); this._more = new Element();
  const info = new Element();
  this._guidePanel = {querySelector: selector => selector === '.guide-info' ? info : null};
  this.shadowRoot = {querySelector: selector => selector === '.brand-title' ? title : null, appendChild: () => {}};
  this._renderGuide();
};
...
assert.equal(card._onlyGuide, false, 'Default shows all channels, including ones with no EPG');
...
assert.equal(title.children.filter(c => c.className === 'tp-version-badge').length, 1);
assert.equal(title.children.find(c => c.className === 'tp-version-badge').textContent, 'Card v0.3.4');
assert.match(addon, /overflow-y:auto; touch-action:pan-x pan-y/);
assert.match(addon, /\.guide-scroll \{flex:1 1 0; height:0/);
// The first scheduled check must not add rows when there is already overflow.
while (frames.length) frames.shift()();
assert.equal(card._limit, 35);
...
assert.equal(title.children.filter(c => c.className === 'tp-version-badge').length, 1);
while (frames.length) frames.shift()();
// A guide with too few rows to scroll must append another page without a scroll event.
card._scroll.scrollHeight = 200;
card._queueGuideFill();
assert.equal(frames.length, 1);
frames.shift()();
assert.equal(card._limit, 70, 'Short guide automatically grows to become scrollable');
console.log('PASS: overflow styling, guide auto-fill and auto-scroll, stable position, back to top, version badge');
